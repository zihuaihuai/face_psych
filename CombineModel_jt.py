    ...
    def __init__(self):
        ...
        if self.inmodel:
            ...
            for key in self.model.keys():
                ...
                self.mask[key] = cv2.cvtColor(cv2.imread('heat/' + key + '.jpg'), cv2.COLOR_RGB2GRAY).astype(
                    np.float) / 255
        ...
        self.random_ = random.randint(0, self.model['bg'].feature_list[self.sex].shape[0])
                    ...
                    for key_p in self.model.keys():
                        if key_p != '':
                            ...
                            shadow_[loc_p[1]:loc_p[1] + loc_p[2], loc_p[0]:loc_p[0] + loc_p[2], :] = 255 - (
                                    255 - shadow_[loc_p[1]:loc_p[1] + loc_p[2], loc_p[0]:loc_p[0] + loc_p[2], :]) * (
                                                                                                                 1 - (
                                                                                                                 1 -
                                                                                                                 self.mask[
                                                                                                                     key_p]) * 0.2)
