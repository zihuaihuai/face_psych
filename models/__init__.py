...
#
#         self.downsample = nn.Pool(3, stride=2, padding=[1, 1], count_include_pad=False, op='mean')
#
...
#         sequence = [[nn.Conv(input_nc, ndf, kernel_size=kw, stride=2, padding=padw), nn.LeakyReLU(0.2, True)]]
#
#         nf = ndf
...
#         nf = min(nf * 2, 512)
#         sequence += [[
#             nn.Conv(nf_prev, nf, kernel_size=kw, stride=1, padding=padw),
#             norm_layer(nf),
#             nn.LeakyReLU(0.2, True)
#         ]]
#
#         sequence += [[nn.Conv(nf, 1, kernel_size=kw, stride=1, padding=padw)]]
#
#         if use_sigmoid:
#             sequence += [[nn.Sigmoid()]]
#
